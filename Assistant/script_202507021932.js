const API_URL = `https://script.google.com/macros/s/AKfycbxfU45-iI41ZlG3DUQQvosMLGrYYXRKFhHC_ET9PX8pJhoy98qM6HILuHz8xTW0k_3B/exec`;
...
async function loadAddonQA() {
    try {
        ...
        console.log('アドオンQ&A情報が正常にロードされました:', addonQA); // Q&A情報を初期プロンプトとしてchatHistoryに追加
...
function addMessage(text, sender) {
    ...
    chatBox.appendChild(messageDiv); // スクロールを一番下へ
...
async function sendMessageToGemini(message) {
    ...
    try {
        ...
        const geminiResponse = data.candidates[0].content.parts[0].text; // Geminiの応答をチャット履歴に追加し、表示 (タイピング効果を使用) // タイピング完了後にチャット履歴にモデルの応答を追加する
